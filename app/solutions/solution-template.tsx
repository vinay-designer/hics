...
<section className="relative min-h-screen flex flex-col justify-center items-center py-12 md:py-20 px-4">
  <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
    ...
    {/* Abstract background graphic - responsive sizing */}
    <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full h-full -z-5 overflow-hidden">
      <motion.div 
        initial={{ opacity: 0, scale: 0.8 }}
        animate={{ opacity: 1, scale: 1 }}
        transition={{ duration: 1, delay: 0.5 }}
        className="absolute w-64 sm:w-72 md:w-80 lg:w-96 h-64 sm:h-72 md:h-80 lg:h-96 rounded-full border border-orange-200/30 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2"
      />
      <motion.div 
        initial={{ opacity: 0, scale: 0.5 }}
        animate={{ opacity: 1, scale: 1 }}
        transition={{ duration: 1, delay: 0.7 }}
        className="absolute w-48 sm:w-52 md:w-56 lg:w-64 h-48 sm:h-52 md:h-56 lg:h-64 rounded-full border border-blue-200/30 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2"
      />
      <motion.div 
        initial={{ opacity: 0, scale: 0.2 }}
        animate={{ opacity: 1, scale: 1 }}
        transition={{ duration: 1, delay: 0.9 }}
        className="absolute w-24 sm:w-28 md:w-32 h-24 sm:h-28 md:h-32 rounded-full border border-orange-200/30 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2"
      />
      
      {/* Animated dots with responsive positioning */}
      {[...Array(8)].map((_, i) => (
        <motion.div
          key={i}
          className={`absolute w-2 sm:w-3 h-2 sm:h-3 rounded-full ${i % 2 === 0 ? 'bg-orange-500/20' : 'bg-blue-500/20'}`}
          initial={{ x: 0, y: 0, opacity: 0 }}
          animate={{ 
            x: Math.cos(i * Math.PI / 4) * (window.innerWidth < 640 ? 100 : window.innerWidth < 768 ? 150 : 200), 
            y: Math.sin(i * Math.PI / 4) * (window.innerWidth < 640 ? 100 : window.innerWidth < 768 ? 150 : 200),
            opacity: 0.8
          }}
